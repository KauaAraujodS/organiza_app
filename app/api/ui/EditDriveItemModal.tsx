...
export default function EditDriveItemModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button type="button" className="modal-overlay absolute inset-0" onClick={onClose} aria-label="Fechar modal" />
      <div className="modal-surface relative w-full max-w-3xl rounded-2xl p-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="modal-muted text-sm">
              {isFolder ? "Editar pasta" : "Editar arquivo"}
            </div>
            <div className="modal-title text-xl font-semibold">{title}</div>
          </div>

          <button
            onClick={onClose}
            className="app-button-ghost rounded-xl px-4 py-2"
          >
            Fechar
          </button>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div className="md:col-span-2">
            <label className="modal-label text-sm">Nome</label>
            <input
              value={name}
            ...
            />
            <div className="modal-muted mt-1 text-xs">
          ...
          {/* Autor */}
          <div>
            <label className="modal-label text-sm">Autor</label>
            <input
              value={author}
          ...
          <div className="relative">
            <label className="modal-label text-sm">Prioridade</label>
            <button
              type="button"
              onClick={() => setPriorityOpen((v) => !v)}
              className="modal-field mt-2 flex items-center justify-between"
            >
              <span
                className={[
              ...
              <span className="modal-muted text-xs">▼</span>
            </button>
            ...
            {priorityOpen ? (
              <>
                ...
                />
                <div className="modal-menu absolute z-50 mt-2 w-full rounded-2xl p-2 shadow-xl">
                    ...
                    (opt) => (
                      <button
                        ...
                        className={[
                          ...
                          opt === priority ? "bg-white/15" : "hover:bg-white/10",
                        ].join(" ")}
                      >
                        <span
                          className={[
          ...
          {/* Cor */}
          <div>
            <label className="modal-label text-sm">Cor (HEX)</label>
            <div className="mt-2 flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={openPicker}
                className="h-12 w-12 rounded-xl border border-[var(--line-soft)]"
          ...
          <div className="md:col-span-1">
            <label className="modal-label text-sm">Ícone</label>
              <div className="modal-soft-box mt-2 rounded-2xl p-3">
                <div className="modal-muted mb-2 text-xs">
                    ...
                    <button
                      ...
                      className={[
                        ...
                        iconEmoji === ic
                          ...
                          : "border-[var(--line-soft)] bg-[var(--surface-1)] hover:bg-[var(--surface-3)]",
                      ].join(" ")}
                      title={ic}
                      type="button"
                    >
          ...
          {isFolder ? (
            <div className="md:col-span-3">
              <div className="modal-soft-box rounded-2xl p-4">
          ...
          <div className="md:col-span-3">
            <div className="modal-soft-box rounded-2xl p-4">
              ...
              <div className="mt-4">
                <label className="modal-label text-sm">Senha (mín. 4)</label>
                <input
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  disabled={!isLocked}
                  className="modal-field mt-2 disabled:opacity-50"
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          <button
            onClick={onClose}
            className="app-button-ghost rounded-xl px-5 py-3"
            disabled={saving}
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            className="app-button rounded-xl px-6 py-3 font-medium disabled:opacity-60"
            disabled={saving}
          >
