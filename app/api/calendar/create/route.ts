...
export async function POST(req: Request) {
  try {
    const { accessToken, event } = (await req.json()) as {
      accessToken?: string;
      event?: {
        title?: string;
        description?: string;
    ...
    if (event.allDay) {
      ...
      if (
        ...
        // suporta evento atravessando dia (ex: 23:00 -> 01:00)
    ...
    if (!r.ok) {
      return NextResponse.json({ error: data?.error?.message || "Calendar create failed" }, { status: r.status });
    }
